...
**The second tab of [`relian-architecture.html`](relian-architecture.html).** This file is
the readable source; the Atlas renders it as its Technical Summary tab.
...
9. **File inventory, lineage, and target-schema DDL** ← *we are here.* Four new modules:
   ...
   On AWS CardDemo: 13 datasets where FD record length and JCL
   LRECL agree exactly, 0 disagreements, 41
   with no LRECL declared and 194 with no resolvable layout.
...
| v1.2 seal | 29 files, `a8695c2c…`, fp `233bb4406e2de606` | VERIFIED | Ed25519 over the manifest; checked by `verify_manifest.py` every CI run |
...
## 8. Using the Build Atlas

`relian-architecture.html` is **one self-contained file**. No build step, no sibling
files, no external assets but Google Fonts. Open it from disk, email it, or project it;
it is also what `https://khaaliswooden-max.github.io/relian/` serves.

**The two tabs.** *Build Atlas* is the three-plate visual brief; *Technical Summary* is
this document. Both panels are real HTML inside the one file, so the summary is present
with JavaScript disabled and when the page is printed — it is switched, not fetched.

**Moving the position flag.** Click any stop on Plate III and the flag plants there, the
stops behind it fill in, the phase bands advance, and the record panel swaps to that
stop's state. Arrow keys work too; the controls under the panel step forward and back.

That is a view change. The position of record is the `data-current` attribute on `#rail`
in the file — edit it and commit, and the build's position becomes a version-controlled
fact rather than browser state.

**As the build advances**, a newly completed stop needs three edits in
`relian-architecture.html`: its `data-state` on the rail, its panel copy and status
chip, and any row it touches in the provenance appendix. A stop does not move from *not
built* to *measured* until a command has been run and its output recorded — the same
