...
pip install cryptography numpy pandas --break-system-packages -q
...
async def main():
    ...
    assert r1.semantic_score == 100.0, f"in-corpus must be MEASURED 100.0 (C1 core), got {r1.semantic_score}"
    assert r1.test_coverage is None, "coverage must be None in-pipeline (no tool ran)"
    assert r1.attestation_tx, "measured run must be attested (simulated:true)"
    r2=await o.migrate(MigrationConfig('examples/cobol/banking-system.cbl','cobol','java','/tmp/v2'))
    assert r2.status.value == 'failed', "out-of-subset must FAIL, never placeholder-complete"
    assert r2.semantic_score is None and r2.attestation_tx is None
    print('PASS honesty gates: measured 100.0 in-corpus + attested; honest FAIL out-of-subset')
asyncio.run(main())
PY
